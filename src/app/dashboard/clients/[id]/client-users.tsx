...
export default function ClientUsers({ clientId }: ClientUsersProps) {
  ...
  const downloadTemplate = () => {
    const csvContent = [
      ...
      'John Doe,john.doe@example.com,Technology,User',
      'Jane Smith,jane.smith@example.com,Healthcare,User'
    ].join('\n')
    ...
    try {
      ...
      for (let j = 0; j < headerLine.length; j++) {
        ...
        if (char === '"') {
          ...
          headerValues.push(headerCurrent.trim().replace(/^"|"$/g, '').toLowerCase())
          headerCurrent = ''
        } else {
          headerCurrent += char
        }
      ...
      const industryIndex = headerValues.indexOf('industry')
      const roleIndex = headerValues.indexOf('role')
      const usernameIndex = headerValues.indexOf('username') // For backward compatibility
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length < 3) continue // Skip incomplete rows (at least Name, Email, Industry required)
        
        // Parse CSV with flexible column order (handle both old format with Username and new format without)
        
        const user: UserData = {
          Name: (nameIndex >= 0 ? values[nameIndex] : values[0])?.replace(/^"|"$/g, '') || '',
          Email: (emailIndex >= 0 ? values[emailIndex] : values[1])?.replace(/^"|"$/g, '') || '',
          Username: (usernameIndex >= 0 ? values[usernameIndex] : '')?.replace(/^"|"$/g, '') || '', // Optional, will be auto-generated
          Industry: (industryIndex >= 0 ? values[industryIndex] : values[2])?.replace(/^"|"$/g, '') || '',
          Role: (roleIndex >= 0 ? values[roleIndex] : (values[3] || values[4]))?.replace(/^"|"$/g, '') || 'User'
        }
        ...
        // Username is auto-generated, so we don't need to set it here
        // It will be generated by the API
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload a CSV file to create multiple users at once. Required fields are: <strong>Name</strong>, <strong>Email</strong>, and <strong>Industry</strong>. <strong>Role</strong> is optional. Usernames are automatically generated from names.
          </CardDescription>
        </CardHeader>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <table className="min-w-full divide-y divide-gray-200 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    <tr>
                      ...
                      <td className="px-3 py-2 text-gray-500">User</td>
                    </tr>
                    <tr>
              ...
              <div className="bg-gray-50 px-3 py-2 border-t border-gray-200">
                    <p className="text-xs text-gray-500">
                      ...
                      {' • '}
                      <span className="font-medium">Optional:</span> Role
                      {' • '}
